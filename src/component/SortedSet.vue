<template>
  <el-container>
      ...
      <div id="box">
        ...
        <div style="margin-top: 5px">
          ...
          <el-button type="primary" @click="addDialog = true;keyDisable=false"
                     icon="el-icon-document-add" style="margin-left: 25%">单个添加
          </el-button>
    ...
    <el-button type="danger" @click="del(delId)">确 定</el-button>
        ...
        <el-dialog :title="key" :visible.sync="editDialog" :before-close="handleClose">
          <el-button style="margin-bottom: 10px;margin-top: -200px" icon="el-icon-document-add" type="primary"
                     @click="keyDisable = true;addDialog=true;addForm.key = key">添加
          </el-button>
          <el-table
            :data="ZSetVal"
            border
          ...
          <div style="margin-top: 5px;margin-bottom: -20px">
            ...
            <div style="margin-top: 10px;">
              <span style="display:block;text-align:center">总共{{ pageObj.total }}条</span>
            </div>

          </div>
        </el-dialog>

        <el-dialog title="添加ZSet" :visible.sync="addDialog">
          ...
          <el-form ref="form" :model="addForm" label-width="80px" size="mini"
            ...
            <el-form-item size="large">
              <el-button type="primary" @click="add(addForm)">立即创建</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      delId: 0,
      timeOut: {},
      addForm: {key: "", member: "", score: 0, exp: 0},
      keyDisable: true,
      disable: false,
      preDisable: true,
      nextDisable: false,
  ...
  methods: {
    nextPage: function (search, flag, size) {
      ...
      this.axios.get(this.api.ZSetScan, {
    ...
    getZSetVal: function (key) {

      this.axios.get(this.api.ZSetGetByKey + key,
        {
          params: {page: this.pageObj.page, size: this.pageObj.size}
        })
        .then(res => {
          this.ZSetVal = res.data.data
    ...
    del: function (key) {
      ...
      this.axios.delete(this.api.ZSetDeleteByKeys + key)
        .then(res => {
    ...
    add: function (form) {
      if (form.key === undefined || form.key === "" || form.member === "") {
        alert("请输入键值")
        return
      }
      ...
      this.axios.post(this.api.ZSetPost, JSON.stringify(form))
        .then(res => {
    ...
    removeValue: function (val) {
      if (confirm("是否确定要删除?")) {
        this.axios.post(this.api.ZSetRemove, val)
          .then(res => {
            if (res.status === 200) {
              alert(res.data.message)
              this.getZSetVal(val.key)
            }
          })
          .catch(err => {
            alert(err.message)
